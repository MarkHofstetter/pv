...
def run(bat_capacity):
    ...
    return sum, dates, saved
    
(sum, dates, saved) = run(args.capacity)
...
ax[1].set_ylim(0, args.capacity * 1.1)
...
print("netto - sim netto - saved %.2f - %.2f - %.2f" % (sum['feedin']-sum['consumption'], sum['sim_feedin']-sum['sim_consumption'], saved))
# print("Ladezustand %.2f  - direct use %.2f " % (bat_charge, direct_use ))

cap_vs_save = []
for cap in [1, 2.5, 5, 7.5, 10, 12.5, 15, 20, 25, 30, 50]:
    (sum, dates, saved) = run(cap)
    cap_vs_save.append((cap, saved))
...
plt.title('Battery capacity vs kWh used additionally')
plt.xlabel('Battery capacity')
plt.ylabel('kWh used additionally')
plt.grid(True)
